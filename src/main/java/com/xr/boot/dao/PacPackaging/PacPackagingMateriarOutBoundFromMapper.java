...
import com.xr.boot.dao.PacPackaging.provider.PacPackagingMateriarOutBoundFromSql;
...
public interface PacPackagingMateriarOutBoundFromMapper {
     ...
     * 多条件查询包装材料出库记录
     */
    @SelectProvider(type = PacPackagingMateriarOutBoundFromSql.class,method = "findPacPackagingMateriarWhere")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "outboundNumber", column = "outboundNumber"),
            @Result(column = "outboundType", property = "pacOutType",
